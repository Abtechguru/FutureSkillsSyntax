...
import type { User, LoginCredentials, RegisterData, CareerField, MentorshipSession } from '@/types'
...
class ApiService {
  ...
  private logoutAction: any = null
  private refreshTokenAction: any = null
  ...
  private async refreshAccessToken(): Promise<string> {
    ...
    this.refreshTokenPromise = new Promise(async (resolve, reject) => {
      try {
        const refreshTokenValue = this.store?.getState()?.auth?.refreshToken
        ...
        if (this.refreshTokenAction) {
          this.store?.dispatch(this.refreshTokenAction({ accessToken: access_token }))
        }
  ...
  injectStore(store: any, logoutAction: any, refreshTokenAction: any) {
    this.store = store
...
export const injectStore = (store: any, logoutAction: any, refreshTokenAction: any) => {
  apiService.injectStore(store, logoutAction, refreshTokenAction)
}
